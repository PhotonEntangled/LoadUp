              ...
              <TableBody>
                {shipment.items.map((item, index) => (
                  <TableRow key={item.id || index}>
                    <TableCell className="font-medium">{item.sku || item.lotSerialNumber || "N/A"}</TableCell>
...
const TripChargesTab = ({ shipment }: { shipment: ApiShipmentDetail }) => {
  const transporter = shipment.customDetails?.transporter
          ...
          {transporter ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                ...
                <p className="text-sm">{transporter.carrierName || "N/A"}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-muted-foreground">Truck ID</p>
                <p className="text-sm">{transporter.truckId || "N/A"}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-muted-foreground">Trailer ID</p>
                <p className="text-sm">{transporter.trailerId || "N/A"}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-muted-foreground">Driver Name</p>
                <p className="text-sm">{transporter.driverName || "N/A"}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-muted-foreground">Driver Cell</p>
                <p className="text-sm">{transporter.driverCell || "N/A"}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-muted-foreground">MC Number</p>
                <p className="text-sm">{transporter.mcNumber || "N/A"}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-muted-foreground">DOT Number</p>
                <p className="text-sm">{transporter.dotNumber || "N/A"}</p>
              </div>
            </div>
          ) : (
            <p className="text-sm text-muted-foreground">No transporter information available</p>
          )}
        </CardContent>
      </Card>
            ...
            <div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                <div>
                  ...
                  <p className="text-sm">{formatCurrency(tripRate.rate, tripRate.currency)}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-muted-foreground">Drop Charge</p>
                  <p className="text-sm">{formatCurrency(tripRate.dropCharge, tripRate.currency)}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-muted-foreground">Manpower Charge</p>
                  <p className="text-sm">{formatCurrency(tripRate.manpowerCharge, tripRate.currency)}</p>
                </div>
              </div>
