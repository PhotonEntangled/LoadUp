...
async function main() {
  try {
    ...
    // Convert dependency scripts to ESM
    console.log(`${colors.blue}Converting dependency scripts to ESM format...${colors.reset}`);
    await convertScriptsToESM();
    
    // Step 1: Update imports
    console.log(`${colors.blue}Step 1: Updating component imports...${colors.reset}`);
    await execPromise('node scripts/update-map-component-imports.mjs', '🔄');
    
    // Step 2: Clean up deprecated files
    console.log(`${colors.blue}Step 2: Cleaning up deprecated files...${colors.reset}`);
    await execPromise('node scripts/cleanup-deprecated-map-files.mjs', '🧹');
...
async function convertScriptsToESM() {
  const scripts = [
    'scripts/update-map-component-imports.js',
    'scripts/cleanup-deprecated-map-files.js'
  ];
  ...
  for (const script of scripts) {
    const esmScript = script.replace('.js', '.mjs');
    ...
    try {
      ...
      if (fs.existsSync(script)) {
        ...
        // Write the new file
        fs.writeFileSync(esmScript, content, 'utf8');
        console.log(`${colors.green}Converted ${script} to ESM format at ${esmScript}${colors.reset}`);
      } else {
        console.warn(`${colors.yellow}Script ${script} not found, skipping conversion${colors.reset}`);
      }
    } catch (error) {
      console.error(`${colors.red}Error converting ${script}: ${error.message}${colors.reset}`);
    }
  }
}
