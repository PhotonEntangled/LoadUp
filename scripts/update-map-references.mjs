...
const REPLACEMENTS = [
  ...
  // Props interface
  {
    find: /FleetOverviewMapV2Props/g,
...
function main() {
  ...
  // Create backup of FleetOverviewMapV2.tsx if it exists
  const fleetOverviewPath = path.resolve(process.cwd(), './src/components/map/FleetOverviewMapV2.tsx');
  if (fs.existsSync(fleetOverviewPath)) {
    ...
    const backupPath = path.join(backupDir, `FleetOverviewMapV2.tsx.${timestamp}.deprecated`);
    
    fs.copyFileSync(fleetOverviewPath, backupPath);
    console.log(`Created backup of FleetOverviewMapV2.tsx at ${backupPath}`);
  }
