  ...
  return (
    <div className={`customer-tracking-view ${className}`} style={{ height, width }}>
      ...
      <div className="map-container relative rounded-md overflow-hidden" style={{ height: 'calc(100% - 140px)' }}>
        ...
        {isMapLoaded && mapRef.current && stops.length > 0 && (
          <MapboxRoute
            map={mapRef.current}
        ...
        {/* Vehicle marker if showing and available */}
        {isMapLoaded && mapRef.current && showVehicleLocation && vehicle?.currentLocation && (
          <MapboxMarker
            map={mapRef.current}
            id={vehicle.id}
            ...
            description={`${vehicle.type} • ${vehicle.licensePlate}`}
            markerType="vehicle"
            status="pending"
            styleOptions={{
              color: '#0088ff'
