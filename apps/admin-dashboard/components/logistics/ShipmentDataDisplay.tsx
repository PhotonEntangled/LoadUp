...
export default function ShipmentDataDisplay({ 
  ...
  const [filteredShipments, setFilteredShipments] = useState<ShipmentData[]>(shipments);
  ...
  const [editableShipments, setEditableShipments] = useState<ShipmentData[]>([]);
  const [changeHistory, setChangeHistory] = useState<ShipmentChange[]>([]);
  
  // Extract all unique sheet names from shipments
  ...
  const statistics = useMemo(() => {
    // For all shipments
    const totalWeight = shipments.reduce((sum, shipment) => sum + (shipment.totalWeight || 0), 0);
    const totalItems = shipments.reduce((sum, shipment) => sum + (shipment.items?.length || 0), 0);
    ...
    // Sheet distribution
    const sheetDistribution = shipments.reduce((acc, shipment) => {
    ...
    const urgentCount = shipments.filter(shipment => {
    ...
    const heavyShipmentCount = shipments.filter(shipment => 
  ...
  const handleSelectShipment = (shipment: ShipmentData, selected: boolean) => {
  ...
  const isSelected = (shipment: ShipmentData) => {
    const identifier = shipment.loadNumber || shipment.orderNumber || '';
  ...
  const handleExportSingle = (shipment: ShipmentData) => {
    // This would use a proper CSV export utility
    alert(`Export single shipment: ${shipment.loadNumber || shipment.orderNumber || 'unknown'}`);
  };
  
  // Calculate estimated delivery time
  const getEstimatedDeliveryTime = (shipment: ShipmentData): string => {
  ...
  const calculateDistance = (shipment: ShipmentData): number => {
  ...
  const handleEditField = (
    ...
    if (field.startsWith('miscellaneousFields.')) {
      ...
      const oldValue = shipment[field as keyof ShipmentData] as string | number | undefined;
